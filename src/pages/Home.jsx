...
export default function ChristmasHome() {
  // USANDO TU CÓDIGO ORIGINAL - NO CAMBIO NADA DE LA LÓGICA
  ...
  const handleQuickAdd = (product, e) => {
    ...
    // USAR TU FUNCIÓN addToCart ORIGINAL
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950' : 'bg-gradient-to-br from-sky-50 via-white to-blue-50'} relative overflow-hidden transition-colors duration-500`}>
      ...
      {/* ❄️ EFECTOS NAVIDEÑOS - Nieve cayendo */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        {/* Gradientes navideños de fondo */}
        <div className={`absolute top-0 right-0 w-96 h-96 ${isDark ? 'bg-red-600/10' : 'bg-red-200/30'} rounded-full blur-3xl animate-pulse`}></div>
        <div className={`absolute bottom-0 left-0 w-96 h-96 ${isDark ? 'bg-green-600/10' : 'bg-green-200/30'} rounded-full blur-3xl animate-pulse`} style={{ animationDelay: '1s' }}></div>
        
        {/* Copos de nieve cayendo */}
        {[...Array(25)].map((_, i) => (
          <div
            key={`snow-${i}`}
            className={`absolute text-2xl ${isDark ? 'text-white/30' : 'text-blue-300/50'}`}
            style={{
        ...
        {[...Array(15)].map((_, i) => (
          <div
            key={`star-${i}`}
            className={`absolute ${isDark ? 'text-yellow-300/40' : 'text-yellow-400/50'}`}
            style={{
              left: `${Math.random() * 100}%`,
              top: `${Math.random() * 100}%`,
        ...
        <div className={`absolute top-32 left-[5%] text-5xl animate-bounce ${isDark ? 'opacity-10' : 'opacity-20'}`} style={{animationDuration: '3s'}}>🎄</div>
        <div className={`absolute top-[20%] right-[8%] text-4xl animate-bounce ${isDark ? 'opacity-10' : 'opacity-20'}`} style={{animationDuration: '4s', animationDelay: '1s'}}>🎁</div>
        <div className={`absolute top-[40%] left-[12%] text-4xl animate-bounce ${isDark ? 'opacity-8' : 'opacity-15'}`} style={{animationDuration: '3.5s', animationDelay: '0.5s'}}>⭐</div>
        <div className={`absolute top-[60%] right-[15%] text-4xl animate-bounce ${isDark ? 'opacity-10' : 'opacity-20'}`} style={{animationDuration: '4.5s', animationDelay: '2s'}}>🔔</div>
        <div className={`absolute bottom-[30%] left-[10%] text-3xl animate-bounce ${isDark ? 'opacity-8' : 'opacity-15'}`} style={{animationDuration: '3.8s', animationDelay: '1.5s'}}>🎅</div>
        <div className={`absolute bottom-[50%] right-[5%] text-5xl animate-bounce ${isDark ? 'opacity-10' : 'opacity-20'}`} style={{animationDuration: '4.2s'}}>🎄</div>
        <div className={`absolute top-[15%] right-[25%] text-4xl animate-bounce ${isDark ? 'opacity-10' : 'opacity-20'}`} style={{animationDuration: '3.3s', animationDelay: '0.8s'}}>🎀</div>
        <div className={`absolute bottom-[15%] right-[30%] text-4xl animate-bounce ${isDark ? 'opacity-10' : 'opacity-20'}`} style={{animationDuration: '4.8s', animationDelay: '1.2s'}}>🎄</div>
      </div>
      ...
      <button
        ...
        className={`fixed top-6 right-6 z-50 w-16 h-16 rounded-full ${
          isDark 
            ? 'bg-gradient-to-br from-red-600 to-green-600 shadow-xl shadow-red-500/30' 
            : 'bg-gradient-to-br from-blue-400 to-sky-300 shadow-xl shadow-blue-400/30'
        } flex items-center justify-center text-3xl hover:scale-110 transition-all duration-300 backdrop-blur-xl border-2 ${isDark ? 'border-red-500/30' : 'border-blue-300/50'}`}
      ...
      <section className="relative overflow-hidden">
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20">
          ...
          <div className={`absolute top-10 left-10 text-6xl animate-bounce ${isDark ? 'opacity-10' : 'opacity-20'}`} style={{animationDuration: '3s'}}>🎄</div>
          <div className={`absolute bottom-10 right-10 text-5xl animate-bounce ${isDark ? 'opacity-10' : 'opacity-20'}`} style={{animationDuration: '3.5s', animationDelay: '1s'}}>🎁</div>
          <br />
          <br />
          <div className="text-center space-y-6 z-10 relative">
            <div className={`inline-flex items-center gap-2 px-4 py-2 ${
              ...
              <Sparkles className={`w-4 h-4 ${isDark ? 'text-red-400' : 'text-red-600'} animate-pulse`} />
              <span className={`text-sm font-semibold ${isDark ? 'text-red-300' : 'text-red-700'}`}>
                🎄 Especial Navidad 2024 🎄
              </span>
            </div>

            <h1 className={`text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold leading-tight ${isDark ? 'text-white' : 'text-gray-900'}`}>
              Loyola{' '}
            ...
            <p className={`text-lg md:text-xl max-w-2xl mx-auto ${isDark ? 'text-gray-400' : 'text-gray-700'}`}>
              ✨ Descubre nuestros diseños especiales de Navidad ✨<br/>
              Bordados personalizados perfectos para regalar
            </p>

            <div className="flex flex-wrap gap-4 justify-center">
              <a 
                href="/productos"
                className={`px-8 py-4 bg-gradient-to-r from-red-600 to-green-600 text-white rounded-xl font-semibold hover:shadow-lg transition-all duration-300 flex items-center gap-2 ${
                  isDark ? 'hover:shadow-red-500/50' : 'hover:shadow-red-400/40'
                ...
                <a
                  ...
                  className={`group rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-2xl cursor-pointer ${
                    isDark 
                      ...
                      : 'bg-white border border-gray-200 hover:border-red-400/50 shadow-sm'
                  } relative`}
                >
                  ...
                  {/* Copos de nieve en hover */}
                  <div className={`absolute top-2 left-2 text-xl opacity-0 group-hover:opacity-60 transition-opacity z-20 ${isDark ? 'text-white' : 'text-blue-400'}`}>❄️</div>
                  <div className={`absolute top-2 right-12 text-xl opacity-0 group-hover:opacity-60 transition-opacity z-20 ${isDark ? 'text-white' : 'text-blue-400'}`} style={{animationDelay: '0.3s'}}>❄️</div>
      ...
      <section className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20 z-10">
        {/* Decoración navideña */}
        <div className={`absolute top-10 left-10 text-6xl animate-bounce ${isDark ? 'opacity-10' : 'opacity-20'}`} style={{animationDuration: '3s'}}>🎄</div>
        <div className={`absolute top-20 right-20 text-5xl animate-bounce ${isDark ? 'opacity-10' : 'opacity-20'}`} style={{animationDuration: '4s', animationDelay: '1s'}}>🎁</div>
        ...
        <div className="max-w-4xl mx-auto space-y-8 relative z-10">
          <div className={`p-8 md:p-12 rounded-3xl ${isDark ? 'bg-slate-900/50 border border-slate-800' : 'bg-white border border-gray-200'} backdrop-blur-xl shadow-xl relative overflow-hidden`}>
            <div className={`text-lg md:text-xl leading-relaxed ${isDark ? 'text-gray-300' : 'text-gray-700'} space-y-6 relative z-10`}>
              <p className="text-center text-2xl md:text-3xl font-bold bg-gradient-to-r from-red-500 to-green-500 bg-clip-text text-transparent mb-6">
                Desde 2018, creando magia bordada
              </p>
              
              <p>
                En <span className="font-bold text-red-500">Loyola</span>, transformamos tus ideas en <span className="font-bold text-green-500">bordados personalizados de alta calidad</span>. Cada puntada cuenta una historia, cada diseño refleja tu personalidad única.
              </p>
              ...
              <p className="text-center text-xl md:text-2xl font-bold text-green-500 my-8 py-4 border-y-2 border-green-500/30">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              { icon: HeartFilled, label: '100% Boliviano', desc: 'Algodón nacional de primera', color: 'from-red-500 to-pink-500' },
            ].map((item, idx) => {
          ...
          <div className="relative z-10 grid md:grid-cols-2 gap-8 items-center">
            <div>
              ...
              <p className="text-lg text-white/90 mb-8">
                Aprovecha nuestras ofertas exclusivas en bordados personalizados.
              </p>
            ...
            <div className="hidden md:flex items-center justify-center">
              <div className="relative">
                ...
                <div className="absolute -bottom-4 -left-4 w-16 h-16 bg-pink-400/50 rounded-full animate-pulse" style={{ animationDelay: '0.5s' }}></div>
              </div>
            </div>
          </div>
        </div>
          ...
          {[
            {
              ...
              color: isDark ? 'from-red-500 to-orange-500' : 'from-red-400 to-orange-400',
            },
            {
              icon: Award,
